...
export default function SessionTutorial({
  ...
  const rootName = header.title.includes(" — ") ? header.title.split(" — ")[0] : header.title;
  const language = header.subtitle.endsWith("root family")
    ? header.subtitle.replace(/ root family$/, "").toLowerCase()
    : "latin";
  ...
  return (
    <div className="relative">
      {stage === "intro" && (
        ...
        >
          <p>
            This is <span className="font-semibold text-amber-100">{rootName}</span> — a {language} root:
            a seed that hides inside lots of everyday words. One root grows a whole family of words —
            let&apos;s forge your first few together!
          </p>
        </CoachMark>
      )}
