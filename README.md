...
- **Productive recall** — the recall test asks you to complete each sentence with the right word; an honest "I don't know" is always allowed.
...
- **Evidence-based mastery** — a word becomes *Mastered* once you keep recalling it correctly across lessons days apart, then it enters your Mastered vault where you can prove again that you still know it. Recognition alone never counts. Words can *lapse*.
- **Review-load throttling** — when the due backlog is high, the session teaches fewer new words.
- **Spaced repetition** — FSRS scheduling keeps every word alive with short daily rechecks.
...
2. **Reforge** — words you still haven't locked in come back as a quick lesson; re-forge them until they graduate to your Mastered vault.
3. **Weekly test & vault** — take the weekly trouble-word test (one shot, Monday reset) and revisit your Mastered vault to prove you still know each word.
4. Watch your **vocabulary meter**, **erudition level**, **mastery ladder**, and **word garden** grow.

## Mastery ladder

`Unseen → Recognised → Recalled → Produced → Mastered` — and words can fall back to **Lapsed**. The ladder is visible on the garden and stats pages.

## Stack

- **Next.js 16** (App Router, Turbopack) + TypeScript + Tailwind CSS v4
- **SQLite** via `better-sqlite3` (shared via Railway volume; passwordless email login)
